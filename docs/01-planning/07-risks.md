...
| R-16 | Verification messages are delayed, abused, or not delivered. | Medium / High | Use Brevo, rate limit requests, use expiring single-use tokens, and provide safe retry feedback. | Keep the account restricted and investigate delivery without weakening verification. |
